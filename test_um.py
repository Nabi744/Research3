...
class Tester:
    ...
    def loss(self,model_list:list,target_list:list,tolerance:float=0.05,w_iteration:float=0.5,w_motion_cost:float=0.5,mode:bool=True)-> float:
        ...
        print(f"iteration_cost:{iteration_cost},motion_cost:{motion_cost}")
        if iteration_cost==np.nan or motion_cost==np.nan:
            return np.nan
        return (w_iteration*iteration_cost+w_motion_cost*(motion_cost))

    def loss1(self,model_list:list,target_list:list,tolerance:float=0.05,a:float=0.5,b:float=0.5,w_iteration:float=0.5,w_motion_cost:float=0.5,mode:bool=True)-> float:
        ...
        print(f"iteration_cost1:{iteration_cost1},motion_cost1:{motion_cost1}")
        if iteration_cost1==np.nan or motion_cost1==np.nan:
            return np.nan
        return (w_iteration*iteration_cost1+w_motion_cost*(motion_cost1))

    def test(self,model_list:list,target_list:list,tolerance:float=0.05,step_w:float=0.05,mode:bool=True)-> DataFrame:
        result=[]
        w_iteration,w_motion_cost=1,0
        total_iteration=int((1 // step_w) + 1)
        present_iteration=0
        while (1>=w_iteration>=0) and (0<=w_motion_cost<=1):
            loss=self.loss(model_list,target_list,tolerance,w_iteration,w_motion_cost,mode)
            result.append([w_iteration,w_motion_cost,loss])
            present_iteration+=1
            print(f"w_iteration:{w_iteration},w_motion_cost:{w_motion_cost},loss:{loss}\niteration:{present_iteration}/{total_iteration}")

            w_iteration+=-step_w;w_motion_cost+=step_w

        return pd.DataFrame(result,columns=['w_iteration','w_motion_cost','loss'])

    def test1(self,model_list:list,target_list:list,tolerance:float=0.05,final_a:float=2,final_b:float=2,step_w:float=0.05,step_a:float=0.05,step_b:float=0.05,mode:bool=True)-> DataFrame:
        result1=[]
        w_iteration,w_motion_cost=1,0
        total_iteration =int((1 // step_w + 1) * ((final_a // step_a + 1) * (final_b // step_b + 1))  )
        present_iteration=0
        while (1>=w_iteration>=0) and (0<=w_motion_cost<=1):
            for a in np.arange(0,final_a,step_a):
                for b in np.arange(0,final_b,step_b):
                    loss1=self.loss1(model_list,target_list,tolerance,a,b,w_iteration,w_motion_cost,mode)
                    result1.append([w_iteration,w_motion_cost,a,b,loss1])
                    present_iteration+=1
                    print(f"w_iteration:{w_iteration},w_motion_cost:{w_motion_cost},a:{a},b:{b},loss1:{loss1}\niteration:{present_iteration}/{total_iteration}")

            w_iteration+=-step_w;w_motion_cost+=step_w

        return pd.DataFrame(result1,columns=['w_iteration','w_motion_cost','a','b','loss1'])

    def optimize1(self,model_list:list,target_list:list,tolerance:float=0.05,final_a:float=2,final_b:float=2,step_w:float=0.05,step_a:float=0.05,step_b:float=0.05,mode:bool=True)-> DataFrame:
        loss1_df=self.test1(model_list,target_list,tolerance,final_a,final_b,step_w,step_a,step_b,mode)
        # find minimum loss1 in loss1_df by a,b
        min_loss1_df=loss1_df.loc[loss1_df.dropna().groupby(['w_iteration','w_motion_cost'])['loss1'].idxmin()]

        return min_loss1_df

    def optimize1_with_result(self,result1_df:DataFrame)->DataFrame:
        return result1_df.loc[result1_df.dropna().groupby(['w_iteration','w_motion_cost'])['loss1'].idxmin()]
